import { node, objectOf, object, number } from 'prop-types';
...
class ChainUnsupportedError extends Error {
...
class ConnectorUnsupportedError extends Error {
...
async function init$1() {
  ...
  } = await import('@web3-react/frame-connector');
  return {
    web3ReactConnector({
      chainId
    }) {
...
async function init$3() {
  const {
    PortisConnector
  } = await import('@web3-react/portis-connector');
  return {
    web3ReactConnector({
      chainId,
      dAppId
...
async function init$4() {
  const {
    ProvidedConnector,
    UserRejectedRequestError
  } = await import('@aragon/provided-connector');
  return {
    web3ReactConnector({
      chainId,
      provider
...
async function init$5() {
  const {
    TorusConnector
  } = await import('@web3-react/torus-connector');
  return {
    web3ReactConnector({
      chainId,
      initOptions,
...
async function init$8() {
  const {
    LedgerConnector
  } = await import('@web3-react/ledger-connector');
  return {
    web3ReactConnector({
      chainId,
      url
...
async function getAccountIsContract(ethereum, account) {
  try {
...
async function getAccountBalance(ethereum, account) {
  return ethereumRequest(ethereum, 'eth_getBalance', [account, 'latest']);
}
async function getBlockNumber(ethereum) {
...
const KNOWN_CHAINS = /*#__PURE__*/new Map([[1, {
  id: 1,
  nativeCurrency: ETH,
  type: 'main',
...
const BASE_URL = 'https://storageapi.fleek.co/aragonone-team-bucket/wallet-icons'; // See the corresponding prop type, EthereumProviderType, in prop-types.js.

const PROVIDERS = /*#__PURE__*/new Map( /*#__PURE__*/[{
  id: 'frame',
  name: 'Frame',
  type: 'Desktop',
...
const NO_BALANCE = '-1';
const UseWalletContext = /*#__PURE__*/createContext(null);
...
function useWalletBalance({
  ...
  useEffect(() => {
    ...
    let cancel = false; // Poll wallet balance

    const pollBalance = pollEvery((account, ethereum, onUpdate) => {
      let lastBalance = NO_BALANCE;
      return {
        async request() {
          return getAccountBalance(ethereum, account).then(value => {
...
function useWatchBlockNumber({
  ...
  const lastBlockNumber = useRef(null); // Using listeners lets useWallet() decide if it wants to expose the block
...
function UseWalletProvider({
  ...
  const connect = useCallback(async (connectorId = 'injected') => {
    ...
    const [connectorInit, connectorConfig] = connectors[connectorId] || []; // Initialize the (useWallet) connector if it exists.

    const connector = await (connectorInit == null ? void 0 : connectorInit()); // Initialize the web3-react connector if it exists.

    const web3ReactConnector = connector == null ? void 0 : connector.web3ReactConnector == null ? void 0 : connector.web3ReactConnector({ ...(connectorConfig || {})
    });
...
export { ChainUnsupportedError, ConnectionRejectedError, ConnectorUnsupportedError, KNOWN_CHAINS, UseWalletProviderWrapper as UseWalletProvider, getProviderFromUseWalletId, getProviderString, useWallet };
